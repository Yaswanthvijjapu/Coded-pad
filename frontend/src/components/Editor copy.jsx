...
const Editor = () => {
  ...
  useEffect(() => {
    const fetchCode = async () => {
      try {
        const res = await axios.get(`https://coded-pad-2myz.vercel.app/api/code/${encodeURIComponent(codeId)}`);
  ...
  const handleSave = async () => {
    try {
      const res = await axios.put(`https://coded-pad-2myz.vercel.app/api/code/${encodeURIComponent(codeId)}`, { code });
